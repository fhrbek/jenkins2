# Puppet RSpec Tests asserting that a catalog for a given host is compliant with
# rules derived from a baseline catalog obtained for host: 'myjenkins'
# Generated by: org.cloudsmith.geppetto.catalog.util.CatalogRspecGenerator
# Generated at: 14.9.2012

require 'spec_helper'

describe 'myjenkins', :type => :host do
  # Set facts to empty hash, and expect them to be injected
  let(:facts) { { } }

  # Classes (in alphabetical order)
  it 'class jenkins' do
    should include_class('jenkins')
  end
  it 'class jenkins::firewall' do
    should include_class('jenkins::firewall')
  end
  it 'class jenkins::package' do
    should include_class('jenkins::package')
  end
  it 'class jenkins::repo' do
    should include_class('jenkins::repo')
  end
  it 'class jenkins::repo::el' do
    should include_class('jenkins::repo::el')
  end
  it 'class jenkins::service' do
    should include_class('jenkins::service')
  end
  it 'class jenkinsserver' do
    should include_class('jenkinsserver')
  end
  it 'class settings' do
    should include_class('settings')
  end

  # Resources per type and title (alphabetically)
  it 'resource anchor jenkins::repo::alpha' do
    should contain_anchor('jenkins::repo::alpha')
  end
  it 'resource anchor jenkins::repo::omega' do
    should contain_anchor('jenkins::repo::omega')
  end
  it 'resource exec rpm --import /etc/yum/jenkins-ci.org.key' do
    should contain_exec('rpm --import /etc/yum/jenkins-ci.org.key').with(
      'path'    => '/bin:/usr/bin',
      'require' => 'File[/etc/yum/jenkins-ci.org.key]',
      'unless'  => 'rpm -q gpg-pubkey-d50582e6-4a3feef6'
    )
  end
  it 'resource file /etc/yum.repos.d/jenkins.repo' do
    should contain_file('/etc/yum.repos.d/jenkins.repo').with(
      'content' => '[jenkins]
name=Jenkins
baseurl=http://pkg.jenkins-ci.org/redhat
gpgcheck=1
',
      'group'   => 'root',
      'mode'    => '0644',
      'owner'   => 'root'
    )
  end
  it 'resource file /etc/yum/jenkins-ci.org.key' do
    should contain_file('/etc/yum/jenkins-ci.org.key').with(
      'content' => '-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1.4.9 (GNU/Linux)


JNBW4snDC4OzjKU6QT386/GA9ssAn3vLzSwn8N1xv5MihWGr5kVzvaE2
=cjdq
-----END PGP PUBLIC KEY BLOCK-----
',
      'group'   => 'root',
      'mode'    => '0644',
      'owner'   => 'root'
    )
  end
  it 'resource package jenkins' do
    should contain_package('jenkins').with(
      'ensure' => 'installed'
    )
  end
  it 'resource package jre' do
    should contain_package('jre').with(
      'ensure' => '1.7.0',
      'noop'   => 'true'
    )
  end
  it 'resource service jenkins' do
    should contain_service('jenkins').with(
      'enable'     => 'true',
      'ensure'     => 'running',
      'hasrestart' => 'true',
      'hasstatus'  => 'true'
    )
  end
  it 'resource stage main' do
    should contain_stage('main').with(
      'name' => 'main'
    )
  end
end
